...
public class ABB<K, V> implements IMapeamento<K, V> {
	...
	Lista<V> recortar(K chaveDeOnde, K chaveAteOnde,No<K, V> raizArvore, Lista<No<K, V>> listaProcessos){
		
	}
	Lista<V> recortar(K chaveDeOnde, K chaveAteOnde){
		if (pesquisar(chaveAteOnde) == null || pesquisar(chaveDeOnde) == null) {
			return null;
		}else{
			Lista<No<K, V>> listaProcessos = new Lista<No<K, V>>();
			return recortar(chaveDeOnde, chaveAteOnde, listaProcessos, this.raiz);
		}
	}
	@Override
	public int tamanho() {
